...
class Truck extends Vehicle implements AbleToTow {
  ...
  // TODO: Implement the tow method from the AbleToTow interface //AM this is using pipe union operator
  tow(vehicle: Truck | Motorbike | Car): void {
    // Check if the vehicle's weight is less than or equal to the truck's towing capacity
    if (vehicle.weight <= this.towingCapacity) {
      console.log(`Truck ${this.make} ${this.model} is towing ${vehicle.make} ${vehicle.model}`);
    } else {
      console.log(`The vehicle ${vehicle.make} ${vehicle.model} is too heavy to be towed by ${this.make} ${this.model}`);
    }
  }
